function [x, y] = plot_rixs(filePath, baseAtom, runList)
    ...
    [x, y] = load_rixs(filePath, baseAtom, runList);
    figure();
    plot(x, y);
...
function data = load_rix(filePath, baseAtom, runNo)
...
function [xdata, sumdata] = load_rixs(filePath, baseAtom, runList)
    ...
    for i = 1:filenumber
        ...
        if i == 1
            refdata = load_rix(filePath, baseAtom, runNo);
            sumdata = refdata;
        else
            uncorrdata = load_rix(filePath, baseAtom, runNo);
